...
type XDSHookClient interface {
	...
	// PostClusterModifyHook provides a way for extensions to modify a cluster generated by Envoy Gateway before it is finalized.
	// Doing so allows extensions to configure/modify cluster fields configured by Envoy Gateway and cluster upstream HTTP filters.
	// PostClusterModify passes a list of Unstructured data for the externalRefs owned by the extension on the HTTPRoute's BackendRefs
	// that created this xDS cluster.
	// PostClusterModify will only be executed if an extension is loaded and only on Clusters which were generated from an HTTPRoute's
	// BackendRefs that uses extension resources as externalRef filters.
